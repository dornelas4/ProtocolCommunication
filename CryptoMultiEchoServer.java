		...
		public void run() {
			try {
				...
				for (;;) {
					...
					echoCipher.init(Cipher.ENCRYPT_MODE, secretKey);
					byte[] echoIV = echoCipher.getIV();
					objectOutput.writeObject(echoIV);
					byte[] encryptEcho = echoCipher.doFinal(str.getBytes());
					objectOutput.writeObject(encryptEcho);
