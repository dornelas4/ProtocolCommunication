        ...
        try {
            ...
            while (!done) {
                ...
                if (userStr.trim().equals("BYE")) {
                    ...
                    //readening echo
                    byte[] echoIV = (byte[]) objectInput.readObject();
                    Cipher echoCipher = Cipher.getInstance("AES/CBC/PKCS5Padding");
                    echoCipher.init(Cipher.DECRYPT_MODE,secretKey,new IvParameterSpec(echoIV));
                    byte[] encryptedEcho = (byte[]) objectInput.readObject();
                    String str = new String(echoCipher.doFinal(encryptedEcho));
                    System.out.println("Encrypted Echo: " + str);
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
